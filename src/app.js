...
function header() {
  return `<header class="world-header"><a class="brand" href="#world"><span class="brand-mark">▤</span><span>BILL QUEST<small>FEDERAL EDITION</small></span></a><div class="top-actions">${button('Bill file', 'file')}${button('Evidence', 'evidence')}${button('Glossary', 'glossary')}${button('Settings', 'settings')}</div></header>`;
}

function builder() {
  return `<section class="builder-intro"><div><p class="eyebrow">A BILL-TO-LAW STRATEGY GAME</p><h1>How much of your bill<br>will survive?</h1><p class="lede">Put your bill together, take it through Congress, and decide which changes you can live with.</p><div class="chips"><span>AP U.S. Government</span><span>Keyboard + trackpad</span><span>First playable · 01</span></div></div>${scenic()}</section><section class="workspace"><div class="builder-heading"><div><p class="eyebrow">01 / EMERGENCY ALERT ACCESSIBILITY</p><h2>What goes in your bill?</h2><p>Choose <strong>three provisions</strong>. Then pick the one you most want to keep.</p></div><span class="count">${state.draft.length} / 3 SELECTED</span></div><div class="cards">${Object.entries(cards).map(([id, c], i) => `<button class="provision ${state.draft.includes(id) ? 'selected' : ''}" data-action="toggle" data-id="${id}" aria-pressed="${state.draft.includes(id)}" ${state.draft.length === 3 && !state.draft.includes(id) ? 'aria-describedby="selection-help"' : ''}><span class="card-top"><span class="card-icon">${c.icon}</span><span class="check">${state.draft.includes(id) ? '✓' : '+'}</span></span><span class="card-number">PROVISION ${String(i + 1).padStart(2, '0')}</span><strong>${c.name}</strong><span class="card-copy">${c.versions[0]}</span><span class="card-reason">${c.reason}</span></button>`).join('')}</div><p id="selection-help" class="hint">To swap a provision, uncheck one first. Each choice gives lawmakers different reasons to support or oppose your bill.</p><div class="draft-footer"><label>Which provision matters most to you? <select id="priority" ${state.draft.length ? '' : 'disabled'}>${state.draft.map(id => `<option value="${id}" ${state.priority === id ? 'selected' : ''}>${cards[id].name}</option>`).join('')}</select></label><div class="basis"><strong>Federal authority</strong><span>Interstate commerce + Necessary and Proper Clause</span></div>${button('Introduce my bill <span aria-hidden="true">→</span>', 'introduce', `class="primary" ${state.draft.length !== 3 ? 'disabled' : ''}`)}</div><details class="disclosure"><summary>How this game works</summary><p>You play as the bill and choose how its supporters respond to changes. The lawmakers still make the official decisions. People, costs, and vote counts are made up for the game.</p><p>This version follows one route through Congress. The vote counts are fixed, so the same choices produce the same results. Real bills can take other routes.</p><p>The president wants lower costs, enough time for upgrades, and accessible alerts across the country. Marking a provision as your priority does not protect it from amendments.</p></details></section>`;
}
...
function controls() {
  ...
  if (state.phase === 'senateRoute') {
    ...
    return `<div class="proposal"><h3>${broad ? 'Senators agree on a timetable.' : 'A senator objects to the agreement.'}</h3><p>${broad ? 'Nobody objects to the proposed timetable. The Senate can reach a passage vote without cloture.' : `A senator still has concerns about how the bill would work. ${id ? `The proposed deal changes this wording: ${textOf(id, 1)}` : 'The sponsors can try cloture to get to a vote.'}`}</p></div><div class="choices">${broad ? button('Use the consent agreement · 1 tick', 'consent', 'class="primary"') : button('Accept the deal · 1 tick', 'negotiate', 'class="primary"')}${button('Ask sponsors to seek cloture · 3 ticks', 'seek')}${broad ? '' : button('Keep the bill as it is; stop here', 'refuse', 'class="danger-text"')}</div>`;
  }
...
function interactObject(o) {
  ...
  if (o.id === 'main') {
    ...
    if (state.phase === 'builder') return startConversation('Legislative counsel', ['Let’s put your bill together. Pick three provisions from the cards on this desk.', 'Because these alerts travel across state lines, Congress can draw on its interstate commerce power. A member of Congress will need to introduce your bill.', 'Pick the provision you most want to keep. Lawmakers may still ask to change it.', 'You will choose how your bill’s supporters respond to proposals. The lawmakers make the official decisions.'], 'draft', 'powers');
...
function openEncounter() {
  ...
  const chamberBanner = (() => {
    if (['committee', 'markup', 'report'].includes(state.phase)) {
...
function dispatch(action) {
  try {
    ...
    conversation = null;
    if (['restart'].includes(action.type)) location = { room: 'corridor', x: 11, y: 11 };
